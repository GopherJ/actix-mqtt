...
mod tests {
    ...
    fn assert_decode_packet<B: AsRef<[u8]> + 'static>(bytes: B, res: Packet) {
        ...
        let mut cur = Bytes::from_static(&bytes[consumed + 1..]).into_buf();
    ...
    static PUBLISH_TEST_DEFAULT: Publish = Publish {
        dup: false,
        retain: false,
        qos: QoS::AtMostOnce,
        topic: String::try_from(Bytes::new()).unwrap(),
